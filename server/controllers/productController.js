...
exports.getProducts = async (req, res) => {
  try {
    const products      = await Product.find({ storeId: req.user._id }).sort({ createdAt: -1 }).limit(500);
...
exports.createProduct = async (req, res) => {
  try {
    const product = new Product({ ...req.body, storeId: req.user._id });
    await product.save();
...
exports.deleteProduct = async (req, res) => {
  try {
    const product = await Product.findOneAndDelete({ _id: req.params.id, storeId: req.user._id });
    if (!product) return res.status(404).json({ success: false, message: 'Product not found' });
...
exports.deleteAllProducts = async (req, res) => {
  try {
    const result = await Product.deleteMany({ storeId: req.user._id });
    res.json({
      success: true,
