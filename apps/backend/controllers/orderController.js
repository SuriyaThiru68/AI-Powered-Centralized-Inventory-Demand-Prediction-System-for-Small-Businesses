...
exports.updateOrder = async (req, res) => {
  try {
    ...
    if (req.body.status === 'Received') {
      const product = await Product.findOne({ name: order.productName, storeId: req.storeId });
      if (product) {
...
exports.deleteOrder = async (req, res) => {
  try {
    const order = await Order.findOneAndDelete({ _id: req.params.id, storeId: req.storeId });
    if (!order) return res.status(404).json({ success: false, message: 'Order not found' });
...
exports.getAISuggestions = async (req, res) => {
  try {
    const products = await Product.find({ storeId: req.storeId, quantity: { $lt: 20 } }).sort({ quantity: 1 }).limit(10);
